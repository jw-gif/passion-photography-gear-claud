-- Update list_open_jobs to include the new approved_job_board status
...
AS $function$
  ...
  WHERE
    ...
    AND pr.status IN ('scheduled', 'in_review', 'approved_job_board')
...
-- Update claim_job to allow claiming on the new approved_job_board status
...
BEGIN
  ...
  IF v_request_status NOT IN ('scheduled', 'in_review', 'approved_job_board') THEN
...
-- Update get_job to allow viewing approved_job_board openings
CREATE OR REPLACE FUNCTION public.get_job(_token text, _opening_id uuid)
 RETURNS TABLE(opening_id uuid, request_id uuid, role photographer_tier, budget_cents integer, event_name text, event_location text, event_date date, event_end_date date, spans_multiple_days boolean, start_time time without time zone, end_time time without time zone, coverage_types photo_coverage_type[], on_site_contact_name text, on_site_contact_phone text, notes text, is_claimed boolean, claimed_by_me boolean)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
